#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build the Catalyst Kodi repository for GitHub Pages.
...
Two roles:
  ...
  * The *repository* add-on - the one-time installer. It only carries a static
    ...
    repository.catalyst.zip at the project root (no source) and a static index.html
...
SERVED = ['plugin.video.catalyst']                # auto-updated through the repo
...
def zip_addon(src_dir, arc_base, out_dir):
    ...
    Old versions are removed - the repo only ever serves the current one."""
    import glob
    addon_id = os.path.basename(src_dir)
    version = version_of(src_dir)
    for old in glob.glob(os.path.join(out_dir, '{0}-*.zip'.format(addon_id))):
        os.remove(old)
...
def install_plugin():
    if not os.path.isdir(WIN_ADDONS):
        ...
        return
    src = os.path.join(PKGS, 'plugin.video.catalyst')
    dst = os.path.join(WIN_ADDONS, 'plugin.video.catalyst')
    if os.path.exists(dst):
        shutil.rmtree(dst)
    shutil.copytree(src, dst, ignore=shutil.ignore_patterns(*EXCLUDE, '*.pyc', '*.zip'))
    print('  installed plugin.video.catalyst -> Kodi (restart Kodi to apply)')


def main():
    args = set(sys.argv[1:])
    print('Building Catalyst repository...')
    for addon_id in SERVED:
