...
Handler modules (ui / playback / trakt_ui) are imported lazily inside _route,
not at module load. Kodi spawns a fresh interpreter for every navigation, so a
top-level import of playback/trakt_ui would make even a static menu pay for the
...
_UI_ACTIONS = frozenset((
    None, 'movies_menu', 'shows_menu', 'genres', 'movies_list', 'shows_list',
    'discover', 'recommendations', 'named', 'years', 'year', 'langs', 'lang',
    'networks', 'network', 'search', 'seasons', 'episodes', 'set_view', 'tools',
))
_TRAKT_UI_ACTIONS = frozenset((
    'boxoffice', 'trakt_menu', 'trakt_continue', 'watched_seeds', 'calendar_menu',
    'calendar', 'trakt_list', 'trakt_lists', 'trakt_list_items', 'trakt_action',
    'trakt_auth', 'trakt_signout',
))


def _route(action, p):
    # Import only the handler module this action needs - keeps static menus off
    # the heavy playback/trakt_ui import paths.
    ...
    if action in _TRAKT_UI_ACTIONS:
